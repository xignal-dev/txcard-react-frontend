...
const IntroForm = ({setState}) => {
  ...
  return (
    <HomeContainer>
      ...
      <TopContainer>
        <c.BigText>{'세계 어디서나 간편한 금융자유\n'}<p style={{color: '#5383FF'}}>{'TRAVEL EXPRESS CARD'}</p></c.BigText>
        
        <MenuBox>
          <MenuItem onClick={() => setState(3)}>{'등록하기'}</MenuItem>
          <MenuItem onClick={() => setState(2)}>{'카드 발급'}</MenuItem>
          <MenuItem onClick={() => setState(4)}>{'분실 신고'}</MenuItem>
        </MenuBox>
